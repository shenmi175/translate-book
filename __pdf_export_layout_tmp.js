    ...
    layout === "bilingual"
      ? blocks
          .map((block) => {
            ...
            return `
              <section class="block">
                <div class="block-meta">${escapeHtml(block.id)} · ${escapeHtml(block.type)}</div>
                <div class="block-grid">
                  ...
                  <div class="pane">
                    ...
                    <pre>${escapeHtml(block.target)}</pre>
                  </div>
                </div>
              </section>
            `;
      ...
      : blocks
          .map((block) => {
            ...
            return `
              <section class="block">
                <div class="block-meta">${escapeHtml(block.id)} · ${escapeHtml(block.type)} · ${escapeHtml(block.shouldTranslate ? block.status : "skipped")}</div>
...
function buildPageHeader(page, layout, pageNumber, pageCount) {
  ...
  commands.push(
    pdfText(
      layout === "bilingual" ? "Left column: source / Right column: translation" : "Translation view",
      PAGE.marginX,
      PAGE.height - 68,
      9,
      COLORS.muted
...
function paginateTranslationOnly(task, renderableBlocks, generatedAt) {
  ...
  for (const block of renderableBlocks) {
    ...
    const meta = `${block.id} · ${block.type} · ${block.shouldTranslate ? block.status : "skipped"}`;
    const textHeight = lines.length * block.style.lineHeight;
    const boxHeight = textHeight + 18;
    const needed = 12 + boxHeight + block.style.gapAfter;

    ensureSpace(needed);
    current.commands += pdfText(meta, PAGE.marginX, current.cursorY, block.style.metaSize, COLORS.muted);
    current.cursorY -= 12;
...
function paginateBilingual(task, renderableBlocks, generatedAt) {
  ...
  const columnWidth = (totalWidth - PAGE.gutter) / 2;
  const leftX = PAGE.marginX;
  const rightX = PAGE.marginX + columnWidth + PAGE.gutter;
  ...
  for (const block of renderableBlocks) {
    ...
    const sourceLines = wrapText(block.source, columnWidth, block.style.fontSize);
    const targetLines = wrapText(block.target, columnWidth, block.style.fontSize);
    const meta = `${block.id} · ${block.type}`;
    const bodyHeight = Math.max(sourceLines.length, targetLines.length) * block.style.lineHeight;
    const boxHeight = bodyHeight + 24;
    const needed = 14 + boxHeight + block.style.gapAfter;
    ...
    if (block.style.panel) {
      current.commands += pdfRectFill(leftX - 4, current.cursorY - bodyHeight - 12, columnWidth + 8, boxHeight, block.style.panel);
      current.commands += pdfRectFill(rightX - 4, current.cursorY - bodyHeight - 12, columnWidth + 8, boxHeight, block.style.panel);
    }

    current.commands += pdfText("Source", leftX, current.cursorY, 8.5, COLORS.accent);
    current.commands += pdfText("Translation", rightX, current.cursorY, 8.5, COLORS.accent);
    ...
    for (const line of sourceLines) {
      current.commands += pdfText(line, leftX, leftY, block.style.fontSize, COLORS.ink);
      leftY -= block.style.lineHeight;
    }

    let rightY = current.cursorY - 16;
    for (const line of targetLines) {
      current.commands += pdfText(line, rightX, rightY, block.style.fontSize, COLORS.ink);
      rightY -= block.style.lineHeight;
    }

    current.cursorY = current.cursorY - bodyHeight - block.style.gapAfter - 8;
  }
